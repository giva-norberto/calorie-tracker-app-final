...
export default function useCalorieTrackerFirebase() {
    ...
    // Carrega os dados do perfil uma única vez
    useEffect(() => {
        if (!user?.uid) {
            setLoading(false);
            return;
        ...
        const loadInitialProfile = async () => {
            setLoading(true);
            try {
                const userProfile = await getDocument(`users/${user.uid}/profile`, 'info');
                if (userProfile) {
    ...
    const calculations = useMemo(() => {
        ...
        else if (bmi >= 25) bmiCategory = 'Sobrepeso';
